...
export default function AdminSportsPage() {
  const [items, setItems] = useState<sportsService.SportItem[]>([]);
  ...
  const filtered = useMemo(() => {
    return items.filter((item) => {
      const matchesSearch = [item.title, item.type, item.venue].some((value) =>
  ...
  const handleDelete = async () => {
    ...
    setConfirming(true);
    await sportsService.remove(deleteTarget);
  ...
  return (
    <div className="space-y-8">
      <PageTitle
        title="Sports"
        subtitle="Manage sport programs and activity cards across the site."
      />
      ...
      <div className="grid gap-4 sm:grid-cols-[1.2fr_0.8fr]">
        <SearchBar
          value={search}
          onChange={setSearch}
          placeholder="Search sport events, types, venues..."
        />
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
        <p className="text-sm text-slate-600">
          Showing {filtered.length} sport item{filtered.length === 1 ? "" : "s"}
          .
        </p>
        <Link
      ...
      {loading ? (
        ...
        <EmptyState
          title="No sports entries"
          description="Create a new sport program or adjust the filter to find existing items."
          actionLabel="Create sport"
          onAction={() => window.location.assign("/admin/sports/create")}
        />
      ) : (
        <DataTable
          columns={[
          ...
          data={filtered}
          onEdit={(item) =>
            window.location.assign(`/admin/sports/edit/${item.id}`)
          }
      ...
      <ConfirmDeleteModal
        ...
        title="Delete sport program"
        message="Deleting this item will remove it from the public sports directory. Continue?"
