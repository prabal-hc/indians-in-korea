...
export default function AdminEventsPage() {
  const [events, setEvents] = useState<eventsService.EventItem[]>([]);
  ...
  const filtered = useMemo(() => {
    return events.filter((item) => {
      const matchesSearch = [item.title, item.category, item.location].some(
  ...
  const handleDelete = async () => {
    ...
    setConfirming(true);
    await eventsService.remove(deleteTarget);
    setEvents((current) => current.filter((item) => item.id !== deleteTarget));
  ...
  return (
    <div className="space-y-8">
      <PageTitle
        title="Events"
        subtitle="Manage event listings, publish statuses, and scheduling details."
      />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Try adjusting your search terms or create a new event to populate the list."
          actionLabel="Create an event"
          onAction={() => window.location.assign("/admin/events/create")}
        />
      ) : (
        <DataTable
          columns={[
          ...
          data={filtered}
          onEdit={(item) =>
            window.location.assign(`/admin/events/edit/${item.id}`)
          }
      ...
      <ConfirmDeleteModal
        ...
        title="Delete event"
        message="This event will be removed from the dashboard. Are you sure you want to continue?"
